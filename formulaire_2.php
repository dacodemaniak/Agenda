			...
			$("#contact").on("submit", function(){ // A la survenue de l'évément "submit" sur l'élément dont l'ID est contact, on traite plusieurs opérations
				...
				$(".required").each(function(){ // boucle sur tous les éléments du document qui portent la classe .required, en exécutant une fonction à chaque fois
					...
					if($(this).val() == ""){
						...
						//$(this).prev("label").css("color","red"); // Parcourir le DOM à la recherche de l'élément <label> précédent l'objet courant
						//$(this).prev("label").addClass("error");
						//$(this).prev("label").addClass("animated shake");
						$(this).parent(".form-group").addClass("animated shake");
				...
				if(!formulaireValide){
					...
					// Il faut enlever la classe animated shake de tous les éléments
					$(".required").each(function(){
						console.log("Retire le classe sur l'élement");
						if($(this).parent(".form-group").hasClass("animated shake")){
							$(this).parent(".form-group").removeClass("animated shake");
						}
					});
				}
